...
type instrumentFailMeter struct {
	noop.Meter
	failOn map[string]error
}

func (m instrumentFailMeter) fail(kind, name string) error {
	if m.failOn == nil {
		return nil
	}
	return m.failOn[kind+":"+name]
}

func (m instrumentFailMeter) Int64Counter(name string, opts ...metric.Int64CounterOption) (metric.Int64Counter, error) {
	if err := m.fail("Int64Counter", name); err != nil {
		return nil, err
	}
	return m.Meter.Int64Counter(name, opts...)
}

func (m instrumentFailMeter) Float64Histogram(name string, opts ...metric.Float64HistogramOption) (metric.Float64Histogram, error) {
	if err := m.fail("Float64Histogram", name); err != nil {
		return nil, err
	}
	return m.Meter.Float64Histogram(name, opts...)
}

func (m instrumentFailMeter) Int64Histogram(name string, opts ...metric.Int64HistogramOption) (metric.Int64Histogram, error) {
	if err := m.fail("Int64Histogram", name); err != nil {
		return nil, err
	}
	return m.Meter.Int64Histogram(name, opts...)
}

func (m instrumentFailMeter) Int64UpDownCounter(name string, opts ...metric.Int64UpDownCounterOption) (metric.Int64UpDownCounter, error) {
	if err := m.fail("Int64UpDownCounter", name); err != nil {
		return nil, err
	}
	return m.Meter.Int64UpDownCounter(name, opts...)
}

func (m instrumentFailMeter) Float64UpDownCounter(name string, opts ...metric.Float64UpDownCounterOption) (metric.Float64UpDownCounter, error) {
	if err := m.fail("Float64UpDownCounter", name); err != nil {
		return nil, err
	}
...
func (p fixedMeterProvider) Meter(string, ...metric.MeterOption) metric.Meter {
	return p.meter
}

func TestMeterWrapper_InitMetricsErrors(t *testing.T) {
	cases := []struct {
		name    string
		failKey string
	}{
		{name: "http request counter", failKey: "Int64Counter:http_requests_total"},
		{name: "http duration histogram", failKey: "Float64Histogram:http_request_duration_seconds"},
		{name: "http request size histogram", failKey: "Int64Histogram:http_request_size_bytes"},
		{name: "http response size histogram", failKey: "Int64Histogram:http_response_size_bytes"},
		{name: "db connection counter", failKey: "Int64UpDownCounter:db_connections_active"},
		{name: "db query duration histogram", failKey: "Float64Histogram:db_query_duration_seconds"},
		{name: "db query counter", failKey: "Int64Counter:db_queries_total"},
		{name: "active users counter", failKey: "Int64UpDownCounter:active_users"},
		{name: "active sessions counter", failKey: "Int64UpDownCounter:active_sessions"},
		{name: "module count counter", failKey: "Int64UpDownCounter:modules_total"},
		{name: "module healthy counter", failKey: "Int64UpDownCounter:modules_healthy"},
		{name: "memory usage counter", failKey: "Int64UpDownCounter:memory_usage_bytes"},
		{name: "cpu usage counter", failKey: "Float64UpDownCounter:cpu_usage_percent"},
		{name: "goroutine counter", failKey: "Int64UpDownCounter:goroutines_total"},
	}

	for _, tc := range cases {
		t.Run(tc.name, func(t *testing.T) {
			mw := &MeterWrapper{
				meter: instrumentFailMeter{
					Meter:  noop.Meter{},
					failOn: map[string]error{tc.failKey: errors.New("instrument failed")},
				},
			}
			err := mw.initMetrics()
			require.Error(t, err)
			assert.Contains(t, err.Error(), "instrument failed")
		})
	}
}

func TestNewMeterWrapper_PropagatesInitError(t *testing.T) {
	otel.SetMeterProvider(fixedMeterProvider{
		meter: instrumentFailMeter{
			Meter:  noop.Meter{},
			failOn: map[string]error{"Int64Counter:http_requests_total": errors.New("boom")},
		},
	})
	t.Cleanup(func() { otel.SetMeterProvider(noop.NewMeterProvider()) })

	_, err := NewMeterWrapper("broken-service")
	require.Error(t, err)
