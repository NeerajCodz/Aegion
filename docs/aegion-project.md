...
| `project-structure.md` | Roadmap module depth (`mfa`, `passkeys`, `social`, `introspection`, `proxy`, `cli`) | `modules/{mfa,passkeys,social,introspection,proxy,cli}/*` | partial | `passkeys` and `social` now include concrete HTTP service flows (challenge/state lifecycle and provider/credential handling); advanced protocol verification and deeper runtime integration remain in progress. |
