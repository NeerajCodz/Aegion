...
| `timeline.md` | OAuth2/OIDC module coverage | `modules/oauth2/{cmd/server/main.go,handler/handler.go,service/*,store/*}` | partial | Auth code/refresh/client credentials/discovery/userinfo are present; device-code token issuance is TODO and signer/JWKS providers are mock. |
...
| `project-structure.md` | Roadmap module depth (`mfa`, `passkeys`, `social`, `introspection`, `proxy`, `cli`) | `modules/{mfa,passkeys,social,introspection,proxy,cli}/*` | missing | These modules have scaffold contracts and entrypoints, but production business logic/integration depth is still pending. |
